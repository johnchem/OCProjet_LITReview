...
class UserNetwork(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        user = request.user
        followed_users = [x.followed_user for x in
                          models.UserFollows.objects.filter(user=user)]

        # display users who follow the login user
        followers = [x.user.username for x in
                     models.UserFollows.objects.filter(followed_user=user)]
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST["search__button"]:
            ...
            try:
                ...
                print(f'DEBUG : {err}')
                search_messages.append(
                    f"l'utilisateur {user_input} n'existe pas"
                    )
...
class UnfollowUser(LoginRequiredMixin, View):
    ...
    def get(self, request, **kwargs):
        ...
        if request.GET["id"]:
            ...
            relation = models.UserFollows.objects.filter(
                user=user,
                followed_user__id=followed_user
                )[0]
            if relation:
                relation.delete()
